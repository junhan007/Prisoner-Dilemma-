...
public class ThreePrisonersDilemmaTesting {
...
class SoftT4TPlayer extends Player {
        ...
        int selectAction(int n, int[] myHistory, int[] oppHistory1, int[] oppHistory2) {
            ...
            int Opp1Score = 0;
            Opp1Score += oppHistory1[n-1];
            int Opp2Score = 0;
            Opp2Score += oppHistory2[n-1];
                
           if((Opp2Score == 1) || (Opp1Score  == 1)){
               return 1;
           }
               
            
            if ((oppHistory1[n-1]) + (oppHistory2[n-1]) == 2)  {
                return 1;
            } else {
                return oppHistory2[n - 1];
            }
    ...
    class Bummer1 extends Player {
        ...
        int intObserveUnfriendlyalway = 6;
 ...
 /* Gosu the Minion -- Note: Gosu the Minion is a NicePlayer */
    class GosuTheMinion extends NicePlayer {
        ...
        private static final double DEFENSIVE_THRESHOLD = 0.650;
                    ...
                    && opp2CoopProb >= FRIENDLY_THRESHOLD)
 {
    ...
    class Junhan extends Player{
        
        //Senario A - Both other Opponents Cooperate in previous Round
        //Senario B - One opponent Defacts in the previous Round
        //Senario C - Both Opponents Defact In the Previous Round
        
        double Opponent1ReactionForA = 0;
        double Size1A = 0;
        double Score1A = -1;
        double Opponent1ReactionForB= 0;
        double Size1B= 0;
        double Score1B = -1;
        double Opponent1ReactionForC = 0;
        double Size1C= 0;
        double Score1C = -1;
        
        double Opponent2ReactionForA = 0;
        double Size2A = 0;
        double Score2A = -1;
        double Opponent2ReactionForB = 0;
        double Score2B = -1;
        double Size2B = 0;
        double Opponent2ReactionForC = 0;
        double Score2C = -1;
        double Size2C = 0;
        
        int ObserverationRounds = 2; 
        
         int selectAction(int n, int[] myHistory, int[] oppHistory1, int[] oppHistory2) {
         
             if ( n < ObserverationRounds)
             {
               return 0;
             }
             
             //Opponent 1 Reactions 
             else{
             //Senario 1A
             if( myHistory[n-2] + oppHistory2[n-2] == 0 ){
                 
                   
                 Opponent1ReactionForA += (oppHistory1[n - 1] == 0 ? 1 : 0);
                 Size1A++;
                 Score1A = checkSenario(Opponent1ReactionForA ,Size1A); 
            //     System.out.println("Decision 1A" + Score1A);
                 
             }
             
             //Senario 1B
             if( myHistory[n-2] + oppHistory2[n-2] == 1 ){
                 Opponent1ReactionForB +=(oppHistory1[n - 1] == 0 ? 1 : 0);
                 Size1B++;
                 Score1B = checkSenario(Opponent1ReactionForB ,Size1B);     
            //          System.out.println("Decision 1B" + Score1B);
             }
             
             //Senario 1C
              if( myHistory[n-2] + oppHistory2[n-2] == 2 ){
                 Opponent1ReactionForC += (oppHistory1[n - 1] == 0 ? 1 : 0);
                 Size1C++;
                 Score1C = checkSenario(Opponent1ReactionForC ,Size1C); 
             //         System.out.println("Decision 1C" + Score1C);
             ...
             //Senario 2A
             if( myHistory[n-2] + oppHistory1[n-2] == 0 ){
                 Opponent2ReactionForA += (oppHistory2[n - 1] == 0 ? 1 : 0);
                 Size2A++;
                 Score2A = checkSenario(Opponent2ReactionForA, Size2A);
                 
             }
             
             //Scenario 2B
             if( myHistory[n-2] + oppHistory1[n-2] == 1 ){
                 Opponent2ReactionForB +=(oppHistory2[n - 1] == 0 ? 1 : 0);
                 Size2B++;
                 Score2B = checkSenario(Opponent2ReactionForB, Size2B);
                 
             }
             //Senario 2C
              if( myHistory[n-2] + oppHistory1[n-2] == 2 ){
                  
                 Opponent2ReactionForC += (oppHistory2[n - 1] == 0 ? 1 : 0);
                 Size2C++;
                  Score2C = checkSenario(Opponent2ReactionForC, Size2C);
              ...
              //For Opponenent1 
              //Scenario 1A
              if(myHistory[n-1] + oppHistory2[n-1] == 0){
                  ...
                  if(Score1A == -1){
                      ...
                      predictOppo1 = predictAction(Score1A);
                }
              //Scenario 1B
              else if(myHistory[n-1] + oppHistory2[n-1] == 1){
                  if(Score1B == -1){
                      predictOppo1 = "N";
                  }                  
                  predictOppo1 = predictAction(Score1B);
                }
              //Scenario 1C
              else {
                  if(Score1C == -1){
                  ...
                  predictOppo1 = predictAction(Score1C);
                }
              
              //For Opponent2
              //Scenario 2A
              if(myHistory[n-1] + oppHistory1[n-1] == 0){
                  if(Score2A == -1){
                      predictOppo2 = "N";
                  }
                  else                 

                  predictOppo2 = predictAction(Score2A);
                }
              //Scenario 1B
              else if(myHistory[n-1] + oppHistory1[n-1] == 1){
                  if(Score2B == -1){
                      predictOppo2="N";
                  }
                  else                  
                  predictOppo2 = predictAction(Score2B);
                }
              //Scenario 1C
              else {
                  if(Score2C == -1){
                  ...
                  predictOppo2 = predictAction(Score2C);
                }       
              
              String combinePrediction = predictOppo2 + predictOppo1; 
              
              switch(combinePrediction){
                  case "AA" : 
                      return 0;
                  case "AE" :
                      return 1;
         ...
         //Gives a score base on good/bad/neutral
         public double checkSenario(double score , double size){

                 double test = score/size;
            return test;
         
         }
         
          public String predictAction(double score){
              if (score < 0.8){
                  return "E";
              }
            else  if (score >0.7){
   ...
   class Junhan1 extends Player {
       //State of Game ( Self Interest(1) / Partial Collusion(2) / Full Collusion (3) 
       int state  = 3 ; 
       boolean partialcollude = true ;
       boolean opponent1isMe = true ;
       boolean opponent2isMe = true ; 
       boolean StartingCollusion = true;
       int selectAction(int n, int[] myHistory, int[] oppHistory1, int[] oppHistory2) {
           ...
           if(state == 3){
              ...
              return 0;
                  ...
                  if ((opponent2isMe == false)  && (opponent1isMe == false)){
                      state = 1;
                      return 1;
                  }
                  else { 
                      if(StartingCollusion){
                          StartingCollusion = false;
                          if(opponent2isMe)
                              return 0;
                          if(opponent1isMe)
                              return 1;
                          ...
                          if(opponent2isMe){
                              if(oppHistory2[n-1] == myHistory[n-1]){
                                  opponent2isMe = false;
                              }
                          ...
                          if(opponent1isMe){
                              if(oppHistory1[n-1] == myHistory[n-1]){
                                  opponent1isMe = false;
                              }
                              return (1 - myHistory[n-1]); 
    ...
    public static void main(String[] args) {
        ...
        ThreePrisonersDilemmaTesting instance = new ThreePrisonersDilemmaTesting();
        showPayoff();
